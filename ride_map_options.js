...
function estimatePrice(vehicleTypeId) {
  return (5 + Math.random() * 10).toFixed(2);
}

// ETA estimation (dummy)
function estimateETA() {
  return Math.floor(3 + Math.random() * 7);
}
...
function loadVehicleTypes(serviceType, pickupLat, pickupLng) {
  ...
  fetch("get_available_vehicle_types.php", {
    method: "POST",
  ...
  .then(data => {
    ...
    (data.vehicle_types || []).forEach(v => {
      const price = estimatePrice(v.vehicle_type_id);
      ...
      card.innerHTML = `
        <div class="vehicle-info">
          <img src="icons/car_placeholder.png">
          <div>
