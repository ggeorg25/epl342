...
$serviceTypeId        = $_SESSION['selected_service_type'] ?? null; // numeric from taxi form
...
// Get all data using single database connection
...
try {
  ...
  if ($conn) {
    ...
    if ($vehicleTypeId) {
      ...
      $stmt->bindParam(2, $vehicleTypeName, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT, 100);
      $stmt->execute();
      $stmt->closeCursor();
    }
...
<script>
  ...
  const serviceTypeName = sessionData.serviceTypeName || serviceTypeId;
  ...
  document.getElementById('service-type').textContent = serviceTypeName || 'N/A';
  ...
  async function confirmRide(){
    ...
    try {
      ...
      if(rideData.status==='error' && rideData.message && rideData.message.includes('already have a pending')) {
        console.log('User has pending request, showing waiting screen');
        showWaitingScreen();
        return;
      }
